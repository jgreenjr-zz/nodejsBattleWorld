...
function GameIsSetupCorrectly(){
    ...
    testrunner.Assert.IsTrue(g.player1 == player1 && g.player2 == player2, "players not set");
...
function HitHitHurtsBothWithHalfHitDamage(){
    ...
    testrunner.Assert.IsTrue(player1.health==19, "player 1 damage not calculated Correctly");
    testrunner.Assert.IsTrue(player2.health==19, "player 2 damage not calculated Correctly");
}
...
function StunnedPlayerReturnsStunned(){
  ...
  var move = g.GetNextMove(player1);
  testrunner.Assert.IsTrue(g.Stunned == move, "Player should be stunned but "+ move);
}
...
function hittingStunnedPlayerShouldResultInFullDamage(){
  ...
  testrunner.Assert.IsEqual(player2.health,18);
...
testrunner.Test("StunnedPlayerShouldBeUnstunedAfterTurn", function(){
  ...
  testrunner.Assert.IsTrue(!player2.stunned , "player is still stunned");
})
...
testrunner.Test("RunAllMoves", function(){
    ...
    testrunner.Assert.IsEqual(3, results.length);
...
testrunner.Test("Game is Over", function(){
    ...
    testrunner.Assert.IsTrue(g.IsOver(), "Not Over");
})
...
testrunner.Test("Game results: Player 1", function(){
    ...
    testrunner.Assert.IsEqual(player1.name+" has been defeated!", g.GetResults());
});
...
testrunner.Test("Game results: Player 2", function(){
    ...
    testrunner.Assert.IsEqual(player2.name+" has been defeated!", g.GetResults());
});
...
testrunner.Test("Game results: Player 1 and 2", function(){
    ...
    testrunner.Assert.IsEqual("undefined and undefined's battle has ended in a draw", g.GetResults());
});
